<script>
	...
	function upload(img) {
		...
		$.ajax({
			url: "{{url('/admin/organizations/ajax-image-upload')}}",
			data: form_data,
			type: 'POST',
			contentType: false,
			processData: false,
		...
		function removeFile() {
			if ($('#file_name').val() != '')
				if (confirm('Вы точно хотите удалить эту картинку?')) {
					...
					form_data.append('_token', '{{csrf_token()}}');
					$.ajax({
						url: '{{url('/admin/organizations/ajax-remove-image')}}' + '/' + $('#file_name').val(),
						data: form_data,
						type: 'POST',
						contentType: false,
						processData: false,
		...
		function removeFileImg() {
			if ($('a.myimg').data('name') != '')
				if (confirm('Вы точно хотите удалить эту картинку?')) {
					...
					form_data.append('_token', '{{csrf_token()}}');
					$.ajax({
						url: '{{url('/admin/organizations/ajax-remove-image')}}' + '/' + $('a.myimg').data('name'),
						data: form_data,
						type: 'POST',
						contentType: false,
						processData: false,
