					...
					<form action="{{route('blog.admin.users.update', $item->id)}}" method="post" data-toggle="validator">
						...
						<div class="box-body">
							...
							<div class="form-group has-feedback">
								...
								<input type="text" class="form-control" name="name" id="name" value="@if(old('name')){{old('name')}} @else{{$item->name ?? ""}} @endif" >
								<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
							</div>
							<div class="form-group has-feedback">
						...
						<div>
							...
							<button type="sumbit" class="btn btn-primary">Сохранить</button>
						</div>
					</form>
				</div>
